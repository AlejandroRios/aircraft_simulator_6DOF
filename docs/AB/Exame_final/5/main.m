...
[Tsol,Xsol] = ode45(@dynamics,0:dt:tf,X_eq,options,U_eq,trim_par.W);
i_t = 1;
Usol = zeros(size(Xsol,1),length(U_eq));
Ysol = zeros(size(Xsol,1),length(Y_eq));
Wsol = zeros(size(Xsol,1),length(trim_par.W));
for u_t=0:dt:tf
    [X,Y,U,W] = dynamics(u_t,Xsol(i_t,:).',U_eq,trim_par.W);
    Usol(i_t,:) = U.';
    Ysol(i_t,:) = Y.';
    Wsol(i_t,:) = W.';
    i_t = i_t+1;
