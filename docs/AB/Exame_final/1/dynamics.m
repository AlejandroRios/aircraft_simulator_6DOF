...
omega_b = [p_rad_s q_deg_s r_rad_s].';
V_b = [u v w].';
...
% Vrel_b = V_b - Vw_b;
% Xaero(1:3) = Vrel_b;
% [Faero_b,Maero_O_b,Yaero] = aero_loads(Xaero,U);
% [Fprop_b,Mprop_O_b,Yprop] = prop_loads(Xaero,U);
...
alfa_dot_deg_s = rad2deg*((u*w_dot-w*u_dot)/(u^2+w^2));
q_dot_deg = rad2deg*edot(5);
theta_dot_deg = rad2deg*Phi_dot_rad_s(2);
H_dot = dREOdt(3);
x_dot = dREOdt(1);
beta_dot_deg_s = rad2deg*((V*v_dot-v*V_dot)/(V*sqrt(u^2+w^2)));
